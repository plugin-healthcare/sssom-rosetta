...
- [x] Construction benchmark script added
      (`scripts/benchmark_omop_construction.py`, rdflib vs. maplib); **not
      yet run against real Athena-scale data** — no licensed OMOP release is
      available in this environment. Numbers still need to be recorded here
      once run against a real release.
- [x] `just typecheck` and the `tests/vocabulary/` suite green; `just lint`
      ...
      `git stash` against the base branch
